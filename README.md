...
需要 Visual Studio 生成工具（勾选"**使用 C++ 的桌面开发**"）+ Windows SDK 10.0.22000+。
MinHook 源码已随包附带，编译过程本身不联网。
...
**如果不想装这几个 GB**，那就别走这条路——本工具只有"自己编译"这一种交付方式
（我不建议你从别人手里拿一个会注入 explorer 的 exe）。回头用「始终合并」或 Windhawk 更省事。
